...
import os
from .utils.helper import process_document_response, save_and_extract_document
...
requests_per_second = 1.0 / config.request_rate_limit if config.request_rate_limit > 0 else 1
session = Session()
...
def extract_companies(csv_file=config.csv_file):
    ...
    try:
        ...
        industry_exclude = ['Services', 'Real Estate', 'Securities & Commodity Futures', 'Banks', 'Insurance']
        df = df[~df["Submitter's industry"].isin(industry_exclude)]
        df.reset_index(drop=True, inplace=True)
        
        tqdm.write(f"Filtered to {len(df)} relevant companies. (exclusions: unlisted companies, NonConsolidated, null names)")
        return df
...
def get_documents_by_date(date_str):
    ...
    params = {
        "date": date_str,
        "type": "2",
        "Subscription-Key": config.api_key
    }
...
def get_documents_by_date_range(start_date, end_date):
    ...
    progress_bar = tqdm(total=len(dates), desc="Fetching list of documents from Get List API endpoint", unit="date")
...
def filter_documents(documents, doc_types=config.target_doc_types):
    ...
    edinet_codes = set(companies['EDINET Code'])  # Using a set for more efficient lookups
    filtered_docs = []
    
    # Use tqdm for progress visualization
    with tqdm(total=len(documents), desc="Filtering documents", unit="doc") as progress_bar:
