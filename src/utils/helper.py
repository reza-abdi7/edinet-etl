...
def cleanup_temp_dir(doc_id):
    ...
    try:
        ...
        logger.error(
            f'Failed to clean up temporary directory for document {doc_id}: {str(e)}'
        )
        return False
...
def save_and_extract_document(content: bytes, doc_info: dict, file_type: str) -> str:
    ...
    final_filename = (
        f"{doc_info['edinetCode']}_{date_str}_{doc_info['docTypeCode']}{extension}"
    )
    ...
    try:
        ...
        logger.error(
            f"Error saving extracted file for document {doc_info['docID']}: {e}"
        )
        return None
