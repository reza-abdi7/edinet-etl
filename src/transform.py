...
def parse_xbrl_file(file_path: str) -> pd.DataFrame:
    ...
    with tqdm(total=total_elements, desc="Parsing XBRL elements", leave=False) as progress_bar:
    ...
    if not records:
        logger.info(f"No revenue data found in XBRL file: {os.path.basename(file_path)}")
...
def select_best_files_by_company(file_paths: list) -> list:
    ...
    with tqdm(total=len(file_paths), desc="Selecting best files", unit="file") as progress_bar:
        for file_path in file_paths:
            ...
            if match:
                ...
                if company_code not in company_files or doc_type == '130':
                    ...
                    if doc_type == '130':
                        logger.info(f"Using corrected file (type 130) for company {company_code}")
            progress_bar.update(1)

    selected_files = list(company_files.values())
    logger.info(f"Selected {len(selected_files)} unique company files from {len(file_paths)} total files")
    return selected_files


def transform_financial_data(file_path: str, company_info: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    for _, row in revenue_data.iterrows():
        ...
        try:
            ...
            logger.info(f"Skipping row with invalid revenue: '{row['値']}' for year {actual_year}")
            continue
...
def process_financial_documents(file_paths: list, company_info: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    dfs = []
    with tqdm(total=len(selected_files), desc="Processing files", unit="file") as progress_bar:
