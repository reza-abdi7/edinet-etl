...
def test_load_to_csv_with_empty_df(tmp_path, empty_df, caplog):
    ...
    assert any("Attempted to load an empty DataFrame" in message for message in caplog.text.splitlines()), \
...
def test_load_to_csv_exception_handling(tmp_path, sample_df, caplog):
    ...
    with patch.object(pd.DataFrame, 'to_csv', side_effect=OSError("Simulated write error")):
        load_to_csv(sample_df, str(output_dir), str(file_name))

    assert any("Error while writing CSV" in message for message in caplog.text.splitlines()), \
